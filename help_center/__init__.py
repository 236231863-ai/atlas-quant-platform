...
class HelpArticle:
    ...
    content:str=""
    def to_dict(self):
        return {"id":self.article_id,"title":self.title}
...
class HelpCenter:
    ...
    def search(self,q):
        return [a for a in self._articles.values() if q.lower() in a.title.lower()]
    def get_by_category(self,c):
        return [a for a in self._articles.values() if a.category==c]
    def export_logs(self):
        return "Log export complete"
    def contact_support(self,msg):
        return f"Support request submitted: {msg}"
    def count(self):
