...
class PlanType(str, Enum): FREE="free"; PRO="pro"; RESEARCH="research"
...
@dataclass
class Subscription: user_id: str; plan: PlanType=PlanType.FREE; features: List[str]=field(default_factory=list); def to_dict(self): return asdict(self)

class PlanService:
    def __init__(self): self._subscriptions: Dict[str, Subscription] = {}
    def register(self, uid: str): self._subscriptions[uid]=Subscription(user_id=uid, plan=PlanType.FREE, features=list(PLAN_FEATURES["free"])); return self._subscriptions[uid]
