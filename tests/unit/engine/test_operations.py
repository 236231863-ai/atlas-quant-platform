...
class TestEval:
    def test_score(self):
        e=IntelligenceEvaluationEngine()
        s=e.evaluate_prediction(0.8,0.7,0.9)
        assert s.overall>0
class TestFeedback:
    def test_action(self):
        u=UserFeedbackEngine()
        u.record_action(UserAction("a1","u1","view_report","r1"))
        assert u.count_actions()==1
class TestMaint:
    def test_check(self):
        m=AutonomousMaintenanceEngine()
        r=m.health_check()
        assert r.health_score>0
class TestReality:
    def test_record(self):
        r=RealityLearningEngine()
        r.record(PredictionRecord("p1","up","up",0.1,"correct"))
        assert r.count()==1
class TestDirector:
    def test_cycle(self):
        d=ResearchDirectorV11()
        r=d.run_operation_cycle()
        assert "health" in r
class TestDashboard:
