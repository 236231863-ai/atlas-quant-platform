...
class CivilizationDashboard:
    def __init__(self): self._data = CivilizationDashboardData()
    def update_timeline(self, timeline: List[Dict[str, Any]]): self._data.research_timeline = timeline
    def update_goal_evolution(self, goals: List[Dict[str, Any]]): self._data.goal_evolution = goals
    def update_knowledge_growth(self, growth: Dict[str, Any]): self._data.knowledge_growth = growth
    def update_agent_generations(self, gens: List[Dict[str, Any]]): self._data.agent_generations = gens
    def update_breakthroughs(self, bts: List[Dict[str, Any]]): self._data.breakthrough_history = bts
    def update_capability(self, caps: List[Dict[str, Any]]): self._data.capability_improvement = caps
