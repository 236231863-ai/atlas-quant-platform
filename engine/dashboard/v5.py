...
class GlobalDashboard:
    def __init__(self): self._data = GlobalDashboardData()
    def update_model_network(self, data: Dict[str, Any]): self._data.model_network = data
    def update_node_status(self, status: Dict[str, Any]): self._data.node_status = status
    def update_agent_activity(self, activity: List[Dict[str, Any]]): self._data.agent_activity = activity
    def update_mission_progress(self, progress: List[Dict[str, Any]]): self._data.mission_progress = progress
    def update_knowledge_exchange(self, exchange: List[Dict[str, Any]]): self._data.knowledge_exchange = exchange
    def update_global_discoveries(self, discoveries: List[Dict[str, Any]]): self._data.global_discoveries = discoveries
