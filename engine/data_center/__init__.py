...
@dataclass
class DataQualityScore: accuracy:float=0.0; completeness:float=0.0; consistency:float=0.0; freshness:float=0.0; reliability:float=0.0
    def overall(self): return round((self.accuracy+self.completeness+self.consistency+self.freshness+self.reliability)/5, 4)
