...
class PlatformReliabilityScore:
    ...
    overall:float=0.0
    def compute(self):
        self.overall=round((self.availability*0.4+(1-self.error_rate)*0.2+(1-min(1.0,self.latency_ms/1000))*0.2+self.recovery_time_min*0.2),4)
    def to_dict(self):
        return asdict(self)
@dataclass
class ModuleHealthReport:
    api_health:float=0.0
    engine_health:float=0.0
    agent_health:float=0.0
    plugin_health:float=0.0
    def to_dict(self):
        return asdict(self)

class ReliabilityEngine:
    def __init__(self):
        self._scores: List[PlatformReliabilityScore] = []
